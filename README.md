...
> **Current status:** SC-010 — organisation-scoped learner blocker and tutor intervention API
...
| SC-009 organisation-scoped session engagement API | Implemented |
| Attendance and understanding-check operations | Implemented |
| Learner self-service and staff authorization | Validated |
| Session-engagement API and PostgreSQL integration tests | Validated |
| SC-010 organisation-scoped blocker and intervention API | Implemented |
| Learner blocker reporting and personal history | Implemented |
| Assigned-tutor and administrator authorization | Validated |
| Blocker lifecycle, intervention and PostgreSQL tests | Validated |
...
### Use the Session Engagement API

Session-engagement requests require a verified bearer token and the authorised organisation context in the `X-Organization-ID` header.

| Resource | Endpoint | Supported methods |
|---|---|---|
| Attendance roster | `/api/v1/catalog/courses/{course_id}/cohorts/{cohort_id}/sessions/{session_id}/attendance` | `GET` |
| My attendance | `/api/v1/catalog/courses/{course_id}/cohorts/{cohort_id}/sessions/{session_id}/my-attendance` | `GET` |
| Learner attendance | `/api/v1/catalog/courses/{course_id}/cohorts/{cohort_id}/sessions/{session_id}/attendance/{learner_id}` | `PUT` |
| Understanding roster | `/api/v1/catalog/courses/{course_id}/cohorts/{cohort_id}/sessions/{session_id}/understanding-checks` | `GET` |
| My understanding check | `/api/v1/catalog/courses/{course_id}/cohorts/{cohort_id}/sessions/{session_id}/my-understanding-check` | `GET`, `PUT` |

Active learners can retrieve only their own attendance and understanding check, and can submit only their own Green, Yellow or Red response. Assigned tutors and administrators can view engagement rosters and record attendance. Engagement writes require a completed session, attendance minutes cannot exceed the session duration, and confidence scores are generated by PostgreSQL.

### Use the Blocker and Intervention API

Blocker and intervention requests require a verified bearer token and the authorised organisation context in the `X-Organization-ID` header.

| Resource                | Endpoint                                                                                                        | Supported methods |
| ----------------------- | --------------------------------------------------------------------------------------------------------------- | ----------------- |
| My blockers             | `/api/v1/catalog/my-blockers`                                                                                   | `GET`             |
| My blocker              | `/api/v1/catalog/my-blockers/{blocker_id}`                                                                      | `GET`             |
| Create my blocker       | `/api/v1/catalog/courses/{course_id}/cohorts/{cohort_id}/my-blockers`                                           | `POST`            |
| Cohort blockers         | `/api/v1/catalog/courses/{course_id}/cohorts/{cohort_id}/blockers`                                              | `GET`             |
| Cohort blocker          | `/api/v1/catalog/courses/{course_id}/cohorts/{cohort_id}/blockers/{blocker_id}`                                 | `GET`, `PUT`      |
| Blocker interventions   | `/api/v1/catalog/courses/{course_id}/cohorts/{cohort_id}/blockers/{blocker_id}/interventions`                   | `POST`            |
| Intervention completion | `/api/v1/catalog/courses/{course_id}/cohorts/{cohort_id}/blockers/{blocker_id}/interventions/{intervention_id}` | `PUT`             |

Active learners can create and review only their own blockers. Assigned active tutors can review and manage blockers in their cohorts, create interventions and complete their own interventions. Administrators can review and manage organisation-scoped blockers. Blocker mutations append audit events atomically, resolved and closed blockers require a resolution summary, and destructive deletion is not exposed.
...
|   |-- SC-009_SESSION_ENGAGEMENT_API.md
|   `-- SC-010_BLOCKER_INTERVENTION_API.md
...
- [SC-009 session attendance and understanding-check API](docs/SC-009_SESSION_ENGAGEMENT_API.md)

* [SC-010 learner blocker and tutor intervention API](docs/SC-010_BLOCKER_INTERVENTION_API.md)
